...
[顺序查找(基于无序链表)](#无序链表中的顺序查找)

[二分查找(基于有序数组)](#有序数组中的二分查找)
...
为了删除一个结点，构造临时4-结点时沿着查找路径**向下**进行变换，还要分解遗留的4-结点时沿着查找路径**向上**进行变换（同插入操作）。

#### 自顶向下的2-3-4树 ####

这是热身操作

2-3-4树的插入算法，允许存在4-结点。该算法沿查找路径**向下**进行变换是为了保证当前结点不是4-结点（这样树底才有空间来插入新的键），沿查找路径**向上**进行变换是为了将之前创建的4-结点配平。

![](image/Transformations-for-insert-in-top-down-2-3-4-trees.png)

#### 删除最小键 ####

从树底部的3-结点中删除键很简单，但2-结点则不然。

从2-结点中删除一个键会留下一个空结点，一般我们会将它替换为一个空链接，但这样会破坏树的完美平衡性。

所以，为了保证不会删除一个2-结点，我们沿着左链接向下进行变换，确保当前结点不是2-结点（可能是3-结点，也可能是4-结点）。

首先，根结点可能有两种情况

1. 若根是2-结点且它的两个子结点都是2-结点，可直接将这三个结点编程一个4-结点；
2. 否则，需要保证根结点的左子结点不是2-结点，若有必要可从它右侧的兄弟结点“借”一个键来。

![](image/Transformations-for-delete-the-minimum.png)

在沿着左链接向下的过程中，保证以下情况之下成立：

- 若 当前结点的左子结点不是2-结点，无需任何操作
- 若 当前结点的左子结点是2-结点 而 它的亲兄弟结点 不是2-结点，将 左子结点的兄弟结点中的一个键 移动到左子结点中。
- 若 当前结点的左子结点 和 它的亲兄弟结点 都是2-结点，将左子结点、父结点中的最小键 和 左子结点最近的兄弟结点合并成为一个4-结点，使父结点 由 3-结点 变为 2-结点 或者 由 4-结点 变为 3-结点。

在遍历的过程中执行这个过程中，最后能够得到一个含有最小键的3-结点或者4-结点，然后可以直接从中将其删除，将3-结点变为2-结点，或者将4-结点变为3-结点。然后再回头向上解析所有临时的4-结点

[红黑树#deleteMin](RedBlackBST.java)

#### 删除操作 ####

在查找路径上进行和删除最小键相同的变换同样可保证在查找过程中任意当前结点均不是2-结点。

若被查找的键在树的底部，我可直接删除它。

若不在，需将它和它的后继结点交换，就和二叉查找树一样。

因为当前结点必然不是2-结点，问题已转化为一棵根结点不是2-结点的子树中删除最小的键，可 复用 上述删除最小键的算法。

和以前一样，删除之后我们需要向上回溯并分解余下的4-结点。

[红黑树#delete](RedBlackBST.java)
...
![](image/Hashing-the-crux-of-the-problem.png)

散列表是算法在时间和空间上作出权衡的经典例子。

对于每种类型的键都需要一个与之对应的散列方法。

将整数散列最常用方法是**除留余数法 modular hashing**。选择大小为素数M的数组，对于任意正整数k，计算 k % M。这方法计算容易且能够有效地将键散布在0到M-1的范围内。
...
![](image/Hash-value-frequencies-for-words-in-Tale-of-Two-Cities.png)
...
![](image/Hashing-with-separate-chaining-for-standard-indexing-client.png)
...
![键表的长度](image/List-lengths-using-SeparateChainingHashST.png)

![](image/Costs-using-SeparateChainingHashST.png)
...
![](image/Trace-of-linear-probing-ST-implementation-for-standard-indexing-client.png)

[基于线性探测法的散列表](LinearProbingHashST.java)

线性探测的平均成本取决于元素在插入数组后聚集成的一组连接条目，也叫做 **键簇Clustering**

![](image/Clustering-in-linear-probing.png)

短小的键簇才能保证较高的效率。

![](image/Table-occupancy-patterns.png)
...
![用自动调整数组大小的功能的散列表](image/cost-with-doubling.png)
